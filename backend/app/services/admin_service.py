...
class AdminService:
    async def list_dlq(self, db: AsyncSession, admin_id: int, page: int, replayed: bool, suppressed: bool, request_for_audit: Any = None) -> Dict[str, Any]:
        await audit(db, AuditAction.ADMIN_DLQ_VIEW, user_id=admin_id,
                    detail={"page": page}, request=request_for_audit)
        offset = (page - 1) * 20
        total = await admin_repo.count_dlq(db, replayed, suppressed)
        items = await admin_repo.list_dlq(db, replayed, suppressed, offset, 20)
        return {
    ...
    async def replay_dlq(self, db: AsyncSession, admin_id: int, entry_id: int, request_for_audit: Any = None) -> Dict[str, Any]:
        await audit(db, AuditAction.DLQ_REPLAY, user_id=admin_id,
    ...
    async def query_audit(self, db: AsyncSession, admin_id: int, page: int, action: Optional[str], user_id: Optional[int], resource_type: Optional[str], request_for_audit: Any = None) -> Dict[str, Any]:
        await audit(db, AuditAction.ADMIN_VIEW_AUDIT, user_id=admin_id,
                    detail={"filters": {"action": action, "user_id": user_id}}, request=request_for_audit)
        offset = (page - 1) * 50
        total = await admin_repo.count_audit(db, action, user_id, resource_type)
        items = await admin_repo.list_audit(db, action, user_id, resource_type, offset, 50)
        return {
    ...
    async def verify_audit(self, db: AsyncSession, admin_id: int, user_id: Optional[int], limit: int, request_for_audit: Any = None) -> Dict[str, Any]:
        result = await verify_audit_chain(db, user_id=user_id, limit=limit)
        await audit(db, AuditAction.AUDIT_CHAIN_VERIFY, user_id=admin_id,
                    detail={"verified_user_id": user_id, "valid": result.get("valid")}, request=request_for_audit)
        return result

    async def verify_global_audit(self, db: AsyncSession, admin_id: int, limit: int, request_for_audit: Any = None) -> Dict[str, Any]:
        result = await verify_global_sequence_integrity(db, limit=limit)
        await audit(db, AuditAction.AUDIT_GLOBAL_VERIFY, user_id=admin_id,
                    detail={"valid": result.get("valid"), "gaps": result.get("gaps_found")}, request=request_for_audit)
        return result

    async def grant_admin(self, db: AsyncSession, admin_id: int, target_id: int, request_for_audit: Any = None) -> Dict[str, Any]:
        target = await admin_repo.get_user(db, target_id)
        if not target:
        ...
        await db.flush()
        await audit(db, AuditAction.ADMIN_GRANT_ADMIN, user_id=admin_id,
    ...
    async def revoke_admin(self, db: AsyncSession, admin_id: int, target_id: int, request_for_audit: Any = None) -> Dict[str, Any]:
        ...
        await db.flush()
        await audit(db, AuditAction.ADMIN_REVOKE_ADMIN, user_id=admin_id,
                    resource_type="user", resource_id=target_id,
                    detail={"revoked_by": admin_id}, request=request_for_audit)
        return {"user_id": target_id, "is_admin": False}

    async def grant_super_admin(self, db: AsyncSession, admin_id: int, target_id: int, request_for_audit: Any = None) -> Dict[str, Any]:
        if target_id == admin_id:
        ...
        target.is_admin = True
        await db.flush()
        await audit(db, AuditAction.ADMIN_GRANT_SUPER, user_id=admin_id,
                    resource_type="user", resource_id=target_id,
                    detail={"granted_by": admin_id, "email": target.email}, request=request_for_audit)
        return {"user_id": target_id, "is_super_admin": True}

    async def list_failed_jobs(self, db: AsyncSession, page: int) -> Dict[str, Any]:
        offset = (page - 1) * 20
        total = await admin_repo.count_failed_jobs(db)
        items = await admin_repo.list_failed_jobs(db, offset, 20)
        return {
