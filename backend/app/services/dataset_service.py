...
class DatasetService:
    ...
    async def list_datasets(self, db: AsyncSession, user_id: int, page: int, page_size: int) -> Dict[str, Any]:
        page_size = min(page_size, 100)
        offset = (page - 1) * page_size
        total = await dataset_repo.count_by_user(db, user_id)
        items = await dataset_repo.list_by_user(db, user_id, offset, page_size)
    ...
    async def get_suggestions(self, db: AsyncSession, dataset_id: int, user_id: int) -> List[Dict[str, Any]]:
        ds = await self.get_dataset(db, dataset_id, user_id)
        return generate_smart_suggestions(ds.profile) if ds.profile else []

    async def compare_datasets(self, db: AsyncSession, id1: int, id2: int, user_id: int) -> Dict[str, Any]:
        ds1 = await dataset_repo.get(db, id=id1)
        ds2 = await dataset_repo.get(db, id=id2)
    ...
    async def get_anomalies(self, db: AsyncSession, dataset_id: int, user_id: int) -> Dict[str, Any]:
        ds = await self.get_dataset(db, dataset_id, user_id)
        df = await download_to_df_async(ds.s3_key, settings.S3_BUCKET_RAW)
        anomalies = detect_anomalies(df)
        return {"anomalies": anomalies, "total_scanned": len(df)}

    async def delete_dataset(self, db: AsyncSession, dataset_id: int, user_id: int, request_for_audit: Any = None) -> None:
        ds = await self.get_dataset(db, dataset_id, user_id)
        await delete_object_async(ds.s3_key, settings.S3_BUCKET_RAW)
        await dataset_repo.remove(db, id=ds.id)
        await audit(db, AuditAction.DATASET_DELETE, user_id=user_id,
